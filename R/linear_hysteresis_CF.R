...
#' Adds water to small depressions
#'
#' @param current_storage 
#' @param delta_storage 
#' @param max_storage 
#' @param current_contrib_frac 
#' @param threshold 
#' @param iterations 
#'
#' @return Returns the updated depressional storage, contributing fraction and outflow
#' @export
#'
#' @examples \dontrun{
#' }
#' 
add_water_to_small <- function(current_storage = 0, 
                              delta_storage = 0, 
                              max_storage = 0, 
                              current_contrib_frac = 0, 
                              threshold = -0.01,
  ...
  if (delta_storage < 0) {
    small_volume <- current_storage + delta_storage 
    outflow <- 0
    if (delta_storage < threshold)
      cf <- 0
    else
      cf <- current_contrib_frac
  } else {
    iterations <- as.integer(iterations)
    iteration_delta <- delta_storage / iterations  
    cf <- current_contrib_frac
    for (i in 1:iterations) {    
      new_cf <- linear_hysteresis_CF(current_storage = 0, 
                                     delta_storage = 0, 
                                     max_storage = 0, 
                                     current_contrib_frac = cf, 
                                     threshold = threshold)
    }
  }
  return(cf, small_volume, outflow)
}